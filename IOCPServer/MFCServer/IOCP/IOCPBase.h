...
private:
	...
	/*
		IOContext 池需要在多线程环境下进行并发访问，因此需要采取同步机制来保证线程安全。
		可以使用线程安全的队列或其他数据结构来存储空闲的上下文对象，并使用互斥锁或原子操作来实现并发访问控制。
		如果池中没有可用的上下文对象，可以根据需要动态扩展池的大小，但要控制池的最大数量，以避免占用过多的系统资源。
...
public:
	...
	IOContext *AllocateIoContext()
	{
		...
		{
			context = contextList.back();//取list最后一个元素
			contextList.pop_back();//删除list最后一个元素
		}
		else	//list为空，新建一个
		{
