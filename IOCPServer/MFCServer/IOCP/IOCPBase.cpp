...
BOOL IOCPBase::InitializeListenSocket()
{
	...
	if (NULL == CreateIoCompletionPort(
		(HANDLE)listenSockContext->connSocket,//需要绑定的socket 
		completionPort,// handle to I/O completion port 完成端口
		(DWORD)listenSockContext,// completion key （完成键）   用socket当完成键 省的后续找socket是谁
		0))
	{
...
BOOL IOCPBase::AssociateWithIOCP(SocketContext * sockContext)
{
	...
	HANDLE hTemp = CreateIoCompletionPort(
		(HANDLE)sockContext->connSocket, //需要绑定的socket
		completionPort, // handle to I/O completion port 完成端口
		(DWORD)sockContext,// completion key （完成键）   用socket当完成键 省的后续找socket是谁
		0);
...
BOOL IOCPBase::PostAccept(SocketContext * sockContext, IOContext * ioContext)
{
	...
	(PASCAL FAR * LPFN_ACCEPTEX)(
		...
		_In_ DWORD dwReceiveDataLength,				将这个设为0？？？
...
BOOL IOCPBase::PostRecv(SocketContext * sockContext, IOContext *ioContext)
{
	...
	int nBytesRecv = WSARecv(
		ioContext->ioSocket,//连接的客户端socket
		&ioContext->wsaBuf,//数据缓冲区
		1, //缓冲区的个数 1说明缓冲区没有定义成数组
		&dwBytes,//接收了多少字节
		&dwFlags,
		&ioContext->overLapped,//必须传入微软规定的重叠结构，我们内容也在其中（相当于夹带私货）
		NULL);
...
BOOL IOCPBase::DoAccpet(SocketContext * sockContext, IOContext * ioContext)
{
	...
	fnGetAcceptExSockAddrs(ioContext->wsaBuf.buf, 0, localAddrLen + 16, clientAddrLen + 16, (LPSOCKADDR *)&localAddr, &localAddrLen, (LPSOCKADDR *)&clientAddr, &clientAddrLen);
	...
	// 4. 将新socket和完成端口绑定
	if (NULL == CreateIoCompletionPort(
		(HANDLE)newSockContext->connSocket,//需要绑定的socket
		completionPort,// handle to I/O completion port 完成端口
		(DWORD)newSockContext,// completion key （完成键）   用socket当完成键 省的后续找socket是谁
		0))
	{
	...
	alive_in.onoff = TRUE;
	alive_in.keepalivetime = 1000 * 10;  // 60s  多长时间（ ms ）没有数据就开始 send 心跳包
	...
	/*
		这里不投递recv请求，结合MFC，通过界面投递send请求
		要求客户端（我们远控的目标机）发送相关数据（比如说屏幕截屏）过来
	*/
...
DWORD IOCPBase::WorkerThreadProc(LPVOID lpParam)
{
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(iocp->stopEvent, 0))
	{
		...
		BOOL bRet = GetQueuedCompletionStatus(
			iocp->completionPort, //hIOCP 创建IOCP对象时候返回的句柄  完成端口
			&dwBytes,//I/O通信的字节数 这次的操作传递了多少个字节的数据
			(PULONG_PTR)&sockContext, //表示完成键（当时socket与IOCP绑定时，当时完成键填的是什么，这里就返回什么）
			&ol, // Pointer to a variable that receives the address of the OVERLAPPED structure 
			INFINITE //等待的时间 INFINITE无限等待
		);
		...
		else
		{
			...
			else
			{
				...
				case RECV_POSTED:
					//表示收包
					//包就在ioContext->wsaBuf.buf中
					iocp->DoRecv(sockContext, ioContext);//获取到了数据，保存在ioContext->wsaBuf.buf
					break;
				case SEND_POSTED:
					//表示发包
